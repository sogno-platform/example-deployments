...
# what does this logger do? it's not on the console
base = os.path.splitext(os.path.basename(sys.argv[0]))[0]
log = logging.getLogger(base)

# read topology from CIM
files = glob.glob('build/temp.linux-*/_deps/cim-data-src/CIGRE_MV/NEPLAN/CIGRE_MV_no_tapchanger_With_LoadFlow_Results/*.xml') # downloaded by CMake
...
csv_files = glob.glob('build/temp.linux-*/_deps/profile-data-src/CIGRE_MV_NoTap/load_profiles/')[0]
...
sim.add_interface(intf_iec104)

# setup simulation exports
for i in range(15):
    objname = 'N'+str(i)
    intf_iec104.export_attribute(sim \
